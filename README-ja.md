...
- 転送の代わりに動作する**HTTPプロキシ**・**SOCKS5プロキシ**モード。上位へチェーンする場合は**`-F -user=`による認証**にも対応。
...
<target>                  ; 通常は <host:port>[/TCP|UDP][/SSL]。他に以下の
  ...
  [proxy | socks]         ; 転送の代わりに<listen>をHTTPプロキシまたは
                          ...
                          ; 要求先へ直接ダイヤルする。-Q/-Z/-M や
                          ; /TCP・/UDP・/SSL サフィックスとは併用不可。
  [<host:port>/proxy]     ; 同様だが、直接ダイヤルせず<host:port>の上位
  [<host:port>/socks]     ; プロキシ/SOCKSサーバー(種別は/proxy->HTTP
                          ; プロキシ、/socks->SOCKS5と一致)へ中継する。
                          ; 上位へのTLS接続は未対応。認証は下記-F参照。
