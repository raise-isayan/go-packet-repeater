...
- 転送の代わりに動作する**HTTPプロキシ**・**SOCKS5プロキシ**モード。上位へチェーンする場合は**`-F -user=`による認証**にも対応。さらに**`listen`側の`/proxy`/`/socks`サフィックスによる両プロトコル間の変換**にも対応(例: クライアントにはSOCKSとして待受けつつ、実際のダイヤル(直接またはチェーン先)はHTTPプロキシとして行う、あるいはその逆)。
...
<target>                  ; 通常は <host:port>[/TCP|UDP][/SSL]。他に以下の
                          ; 2形式があり、転送の代わりにproxy/socksモードを
                          ; 選択する(後述「proxy / socksモード」参照)
  [proxy | socks]         ; 転送の代わりに<listen>をHTTPプロキシまたは
                          ; SOCKS5プロキシとして動作させ、各クライアントの
                          ; 要求先へ直接ダイヤルする。-Q/-Z/-M とは併用不可。
                          ; <listen>側にも独自の"/proxy"/"/socks"サフィックス
                          ; を付与できる(後述「フロントエンド/バックエンド変換」参照)。
                          ; それ以外のサフィックス(/TCP/UDP/SSL)は不可。
  [<host:port>/proxy]     ; 同様だが、直接ダイヤルせず<host:port>の上位
  [<host:port>/socks]     ; HTTPプロキシ/SOCKSサーバーへ中継する。上位への
                          ; TLS接続は未対応。認証は下記-F参照。
...
[option]
  ...
  [-M <MITM>]             ; SSL server MITM option: listenでの接続ごとに
                          ...
                          ; -Z とは併用不可。
  [-F <FORWARD>]          ; forward-proxy auth option: 上位プロキシ/SOCKS
                          ; サーバーへ提示する認証情報。上位チェーン形式
                          ; (<host:port>/proxy または <host:port>/socks)
                          ; が必須で、キーワード単独形式では使えない。
                          ; -Q/-Z/-Mとは無関係(TLSではなく素のプロキシ
                          ; プロトコル認証)。
...
<FORWARD>                 ; -Fのサブオプション
  [-user=<user:pass>]     ; 上位プロキシ/SOCKSサーバーへ提示する認証情報
                          ; (HTTP Basic認証、またはSOCKS5ユーザー名/パスワード
                          ; サブネゴシエーション)。-F使用時は必須で、userは
                          ; 空文字不可。
```

- 位置引数は常に `<target>` → `<listen>` の順(転送先が先、待受アドレスが後)。
- `-Q`/`-Z`/`-M`/`-F` はそれぞれ高々1回だけ指定できるブロックの開始トークンで、
  続く `-key=`/`-cert=`/`-ca=`/`-verify=`(`-M`配下ではさらに`-signca=`/
  `-servername=`)、または`-user=`(`-F`配下)は、次の`-Q`/`-Z`/`-M`/`-F`または
  位置引数が現れるまでそのブロックに属する。これらのサブオプションを単独で
  書くことはできない(エラー)。`-d`/`-dd`/`-ddd`/`-v`/`-help`/`-version`は
  グローバルでブロックの範囲に影響しない。
- オプションは位置引数より前に指定する。`-Q`/`-Z`/`-M`/`-F`ブロック自体は
  任意の順でよく、すべて省略可能。
...
# HTTPプロキシとして8888番で待受け、認証が必要な上位HTTPプロキシへ転送
gopr -F -user=alice:s3cret 192.0.2.11:7777/proxy 8888

# SOCKS5プロキシとして8888番で待受け、認証が必要な上位SOCKSプロキシへ転送
gopr -F -user=alice:s3cret 192.0.2.11:7777/socks 8888

# HTTPプロキシとして8888番で待受け、上位のSOCKSプロキシへチェーン(フロントエンド/バックエンド変換)
gopr 192.0.2.11:7777/socks 8888/proxy

# SOCKS5プロキシとして8888番で待受け、上位のHTTPプロキシへチェーン(フロントエンド/バックエンド変換)
...
| `-F` | forward-proxy authブロックを開始: 上位プロキシ/SOCKSサーバーへの認証情報 | 任意。上位チェーン形式(`<host:port>/proxy`または`<host:port>/socks`)が必須 |
| `-user=<user:pass>`(`-F`配下) | 上位プロキシ/SOCKSサーバーへ提示するユーザー名/パスワード | `-F`使用時は必須。userは空文字不可 |
...
これらのモードは `-Q`/`-Z`/`-M`(およびそのサブオプション)や `/tcp`/`/udp`/`/ssl` サフィックスとは併用できない(`-d`/`-dd`/`-ddd`/`-v` は併用可)。`listen` 側は独自に `/proxy`,`/socks` サフィックスを付与できる — 後述「フロントエンド/バックエンド変換」を参照。

### 上位プロキシへのチェーン(`<host:port>/proxy`, `<host:port>/socks`)

`target` を `<host:port>/proxy` または `<host:port>/socks` の形で指定すると、`listen` で受けた接続を直接宛先へダイヤルせず、指定した上位プロキシ/上位SOCKSサーバー経由で中継する。`listen`側に明示的な`/proxy`/`/socks`サフィックスがなければ、待受モードは上位の種別と常に同じになる(`/proxy` は上位もHTTPプロキシ、`/socks` は上位もSOCKSサーバー)。`listen`側に異なるサフィックスを付与すると、両プロトコル間の変換になる — 後述「フロントエンド/バックエンド変換」を参照。
...
- 上位への接続のTLS化(HTTPSプロキシ経由でのチェーン)は現時点では未対応。上位への接続は常に平文。
- SOCKSは現行実装と同様CONNECTのみ対応(BIND・UDP ASSOCIATE非対応)。上位への転送でもこの制約は変わらない。

### フロントエンド/バックエンド変換(`listen`の`/proxy`・`/socks`サフィックス)

`listen`側にも`/proxy`または`/socks`サフィックスを1つだけ付与できる。これはクライアントに対して待受ける側(フロントエンド)のワイヤプロトコルを指定するもので、`target`側が指定する実際にダイヤルする際のプロトコル(バックエンド: 直接ダイヤル、または`<host:port>/proxy`,`<host:port>/socks`による上位チェーン)とは独立に選べる。両者が一致する場合(`listen`側のサフィックスを省略した場合を含む)は今までどおりの挙動と変わらず、一致しない場合はHTTPプロキシ⇔SOCKSプロキシ間のワイヤプロトコル変換として働く。

```bash
gopr socks 8888/proxy                   # HTTPプロキシとして8888番で待受、直接ダイヤル
gopr proxy 8888/socks                   # SOCKS5プロキシとして8888番で待受、直接ダイヤル

gopr proxy 8888/proxy                   # 以下と同じ: gopr proxy 8888
gopr socks 8888/socks                   # 以下と同じ: gopr socks 8888

gopr 192.0.2.11:7777/socks 8888/proxy   # HTTPプロキシとして8888番で待受、上位のSOCKS(192.0.2.11:7777)へチェーン
gopr 192.0.2.11:7777/proxy 8888/socks   # SOCKS5プロキシとして8888番で待受、上位のHTTPプロキシ(192.0.2.11:7777)へチェーン
```

- `target`は通常のproxy/socks形式(単独キーワード、または`<host:port>/proxy`,`<host:port>/socks`)のいずれかである必要がある。`listen`側の`/proxy`/`/socks`サフィックスはそれらと組み合わせた場合のみ認識され、`/tcp`/`/udp`/`/ssl`とは併用できない。
- 大文字・小文字ポリシーは`proxy`/`socks`キーワードと同様: `listen`側のサフィックスはトークン単位で全て大文字または全て小文字とし、`target`側の大文字/小文字とは独立している。
- CONNECTトンネル要求(HTTP CONNECTまたはSOCKS CONNECT)はフロントエンドのプロトコルで受け取り、バックエンドのプロトコル(直接ダイヤル、またはHTTP CONNECTチェーンかSOCKS5 CONNECTチェーン)で宛先へ接続したうえで、以降は復号せずそのままバイト列を中継する。
- バックエンドがSOCKSにチェーンされたHTTPプロキシのフロントエンドで、CONNECT以外の平文HTTPリクエストを受け取った場合でも、拒否せずそのまま転送する。SOCKS自体はCONNECT相当のトンネルしか表現できないため、宛先へのトンネル(直接またはSOCKS5経由)を確立したうえで、そのトンネル上にHTTPリクエストをそのまま書き込む。
- SOCKSのフロントエンドはもとよりCONNECT相当のリクエストしか送ってこないため、バックエンドが`proxy`種別でも`socks`種別でも挙動に差は生まれない — 同じ宛先へ到達する手段(HTTP CONNECTかSOCKS5 CONNECTか)が異なるだけである。
- `-F -user=`はバックエンドの種別(`target`側)に対して認証し、フロントエンドとは無関係。

### 上位プロキシへの認証(`-F -user=`)

認証が必要な上位プロキシ/SOCKSサーバーへチェーンする場合、`-F -user=<user:pass>`で認証情報を指定する。
